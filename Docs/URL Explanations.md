...
Valid Redirect URIs: http://localhost:9091/login/saml2/sso/keycloak

Master SAML Processing URL:
http://localhost:9091/login/saml2/sso/keycloak

Both settings point to Spring Security's **SAML Assertion Consumer
...
> "After I authenticate the user, where should I send the SAML
> Response?"
...
> "These are the locations to which this SAML client is allowed to have
> authentication results sent."
...
If an incoming SAML `AuthnRequest` asks Keycloak to send the result
...
So you can think of the difference as:
  ...
  **Master SAML          Where Keycloak should normally send the SAML
  Processing URL**       response

  **Valid Redirect       Which response/redirect destinations Keycloak is
  URIs**                 allowed to use

  -----------------------------------------------------------------------

For a simple application with one ACS endpoint, it is completely normal
...
One terminology detail is useful: although Keycloak calls the field
**Master SAML Processing URL**, when talking to SAML administrators you
will usually hear this Spring endpoint called the **ACS URL (Assertion
Consumer Service URL)**. That is the term worth remembering.

# Spring Security Default ACS (Assertion Comsumer Service) URL Pattern

Spring Security has a **default ACS URL pattern**, so normally you don't
invent it yourself.
...
The last part, `keycloak`, is your **registration ID** from
`application.yml`:
...
With your server running on port 9091:
...
Rather than calculating it manually, look at the **Spring Service
Provider metadata**:
...
<AssertionConsumerService
    ...
    Location="http://localhost:9091/login/saml2/sso/keycloak"
    index="0"/>
```
...
One important distinction: **don't try to open the ACS URL directly in
the browser to test it**. The ACS is primarily a POST endpoint where
Keycloak sends a `SAMLResponse`. Opening it with a normal browser GET
isn't a valid SAML login test.
...
**`/saml2/authenticate/{registrationId}` is Spring Security's default
URL for initiating SAML authentication.**
...
and with your application at port 9091:
...
Each one starts authentication using that particular
...
In your case `{registrationId}` is `keycloak`, which is why both URLs
end with `/keycloak`.
...
Yes. Spring Security also provides default URLs for **Service Provider
(SP) metadata**, but metadata is a little different from the login and
ACS endpoints.
...
which **performs an action** (starts login), the metadata URL
**describes your Spring application as a SAML Service Provider**.
...
http://localhost:9091/             http://localhost:9991/
...
> Go to this Keycloak metadata URL to learn how to communicate with and
> trust Keycloak.
...
For your project, I'd memorize these:
  ...
  **ACS --- receive SAML `http://localhost:9091/login/saml2/sso/keycloak`
  response**             

  **Spring SP metadata** `http://localhost:9091/saml2/metadata/keycloak`

  **Keycloak IdP         `http://localhost:9991/realms/MySecurityRealm/protocol/saml/descriptor`
  metadata**             
...
One final difference: `/saml2/authenticate/{registrationId}`,
`/login/saml2/sso/{registrationId}`, and
`/saml2/metadata/{registrationId}` are **Spring Security endpoints**.
The Keycloak `/realms/{realm}/protocol/saml/descriptor` URL is a
**Keycloak endpoint**, not a Spring Security URL.
